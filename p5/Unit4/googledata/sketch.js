...
function setup() {
  ...
  let url = '1JMF56IRjsORGKB4qK8nNYTD38FaDt8FwJ32uIsjTqyc';
  let settings = {
...
function gotData(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    bubbles.push(new Bubble(data[i].Name, data[i].Animal, data[i].Color, data[i].Shape)); // THESE Name and Shape need to match your column names in your spreadsheet!
  }
...
class Bubble {
  ...
  constructor(myName, myAnimal, myColor, myShape) {
    this.name = myName;
  ...
  display() {
    ...
    text(this.shape, this.pos.x, this.pos.y+20);
  }
